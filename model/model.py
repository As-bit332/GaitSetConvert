...
class Model:
    ...
    def collate_fn(self, batch):
        ...
        if self.sample_type == 'random':
            ...
            #print("orignal")
            #print(seqs[0][0].shape)The first two dimension is 1 and 1, batch and feature_num 
            #batch size = 1 all of this is no sense
            seqs = [[
                        np.concatenate([
                                           seqs[i][j]
                                           for i in range(batch_per_gpu * _, batch_per_gpu * (_ + 1))
    ...
    def transform(self, flag, batch_size=1, bin_file_path ="input_data" ,pre_process = False, post_process = False, output_path = "output_data"):
        self.encoder.eval()
        ...
        for i, x in enumerate(data_loader):
            ...
            if post_process == False:
                ...
                feat = np.fromfile(output_path + '/' + f'{i:0>4d}.bin', dtype = np.float32)
                feature = torch.Tensor(feat).float().cpu().view(1, 62, 256)
